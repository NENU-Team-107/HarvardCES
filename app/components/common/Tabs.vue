    ...
    <div class="hidden md:flex justify-center w-full min-h-screen">
        <TabsRoot :default-value="tabMenuBase.at(0)?.content" orientation="vertical" class="flex w-full max-w-7xl">
            <TabsList class="flex flex-col w-60 items-center h-fit sticky top-24 bg-gray-100 rounded-lg shadow-md">
                ...
                <TabsTrigger
v-for="item in tabMenuBase" :key="item.content" class="relative px-8 h-[60px] flex items-center text-base leading-none text-black  select-none
        hover:text-blue-500
        ...
        data-[state=active]:before:translate-x-0" :value="item.content">
                    {{ $t(item.content) }}
                </TabsTrigger>
            </TabsList>
            <slot />
